...
### A-1. 프로덕션 배포를 어떻게 할 것인가 — **막혀 있음**

Vercel 프로덕션은 `main` 에 붙어 있는데, `main` 은 upstream 머지를 쉽게 하려고
원본 커밋 `ac60ef5` 그대로 두고 있다. 브랜치를 푸시했으니 **프리뷰 URL 은 자동으로
잡힌다.** 프로덕션까지 올리려면 셋 중 하나를 골라야 한다.

1. **프리뷰만 쓴다** — `main` 은 그대로. 지금 상태가 이것이다.
2. `trim/menu-sections-components` 를 `main` 에 머지 — 프로덕션 URL 이 생기지만
   upstream 머지가 그만큼 번거로워진다.
3. **Vercel 프로젝트의 프로덕션 브랜치를 `trim/...` 으로 바꾼다** — `main` 을 깨끗이
   두면서 프로덕션 URL 도 얻는다. **이게 제일 낫다고 본다.**
   프로젝트 설정 변경이라 승인 없이 하지 않았다.
...
`apps/v4/examples/base/` 에 데모가 **513개** 있다. 이번에 옮긴 것은 RTL 예제 57개다.
나머지는 아직 영어다. 문서 본문은 한국어인데 미리보기 안 문구는 영어라 섞여 보인다.

- 각 컴포넌트 페이지 맨 위 메인 프리뷰(`*-demo.tsx`)만 70개 — 여기까지가 인수 문서의
  "메인 컴포넌트 프리뷰" 로 읽힌다
- 나머지 386개는 변형 예제(`accordion-basic`, `accordion-multiple` 등)
- `aria`·`radix` 두 벌(약 1,000개)은 손대지 않는 것이 인수 문서 방침
